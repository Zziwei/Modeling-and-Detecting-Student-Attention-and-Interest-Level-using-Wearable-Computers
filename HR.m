...
for k = 3:length(Files(:))
    ...
    disp(FileNames);
    HR_features_list(k - 2,1) = mean(raw_data_mat(1,:));
    HR_features_list(k - 2,2) = std(raw_data_mat(1,:));
    HR_features_list(k - 2,3) = var(raw_data_mat(1,:));
    HR_features_list(k - 2,4) = rms(raw_data_mat(1,:));
...
csvwrite('input_d_2_hrv.csv',input_d);
csvwrite('input_i_2_hrv.csv',input_i);